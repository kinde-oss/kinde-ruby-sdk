...
module KindeApi
  ...
  class CreateConnectionRequestOptionsOneOf2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `KindeApi::CreateConnectionRequestOptionsOneOf2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KindeApi::CreateConnectionRequestOptionsOneOf2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [home_realm_domains, saml_entity_id, saml_acs_url, saml_idp_metadata_url, saml_sign_in_url, saml_email_key_attr, saml_first_name_key_attr, saml_last_name_key_attr, is_create_missing_user, saml_signing_certificate, saml_signing_private_key, is_auto_join_organization_enabled].hash
    end
