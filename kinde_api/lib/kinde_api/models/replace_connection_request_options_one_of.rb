...
module KindeApi
  ...
  class ReplaceConnectionRequestOptionsOneOf
    # Client ID.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `KindeApi::ReplaceConnectionRequestOptionsOneOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KindeApi::ReplaceConnectionRequestOptionsOneOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [client_id, client_secret, home_realm_domains, entra_id_domain, is_use_common_endpoint, is_sync_user_profile_on_login, is_retrieve_provider_user_groups, is_extended_attributes_required].hash
    end
