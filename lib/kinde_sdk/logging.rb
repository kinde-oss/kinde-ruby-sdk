...
module KindeSdk
  ...
  module Logging
    ...
    # 2. @logger instance variable (if set)
    # 3. logger method (if defined)
    # 4. Fallback to $stderr/$stdout based on level
    #
    ...
    def resolve_logger
      if defined?(Rails) && Rails.respond_to?(:logger) && Rails.logger
        Rails.logger
      elsif instance_variable_defined?(:@logger) && @logger
        @logger
      elsif respond_to?(:logger, true) && logger
        logger
      end
    end
  end
